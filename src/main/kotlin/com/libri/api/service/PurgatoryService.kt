...
import com.libri.api.repository.PurgatoryBarcodeRepository
...
class PurgatoryService(
    ...
    fun list(
        pageable: Pageable,
        filter: String?,
    ): Page<PurgatoryBookDTO> {
        val books =
            if (filter.isNullOrBlank()) {
                purgatoryBookRepository.findAllByResolvedIsbnIsNullAndDeletedFalse(pageable)
            } else {
                purgatoryBookRepository.findAllByTitleAndResolvedIsbnIsNullAndDeletedFalse(filter, pageable)
            }
